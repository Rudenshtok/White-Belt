...
void BusforStop(const string& s, const map<string, vector<string>>& bsmap, const vector<string>& busm) { 
    int k = 0; 
    vector<string> v;
    for (const auto& i : bsmap) { 
...
void Stopforbus(const string& s, const map<string, vector<string>>& bsmap, const vector<string>& busm) {
    int l = 0; 
    for (auto i : bsmap) { 
        if (i.first == s) { 
            for (auto m : i.second) { 
